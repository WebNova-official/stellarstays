...
const { startRateSyncScheduler } = require("./services/rateSyncService");
...
mongoose.connect(process.env.MONGODB_URI)
    .then(() => {
        ...
        // Keeps index.html's displayed prices from going stale — pulls live
        // Stayflexi rates into Property.pricePerNight/weekendRate on a timer.
        // Interval configurable via RATE_SYNC_INTERVAL_MS (default 6 hours);
        // set RATE_SYNC_DISABLED=true to turn the background job off entirely
        // (the manual POST /api/sf/sync-rates endpoint still works either way).
        if (process.env.RATE_SYNC_DISABLED !== "true") {
            const intervalMs = Number(process.env.RATE_SYNC_INTERVAL_MS) || 6 * 60 * 60 * 1000;
            startRateSyncScheduler({ intervalMs });
            console.log(`🔄  Rate sync scheduler started (every ${Math.round(intervalMs / 60000)} min)`);
        }
    })
